...
Agent（エージェント+ウォレット） → 商業情報を受信・処理
   │       └── 署名③「この広告を受け取った」（オフチェーン）
   │       → 署名セット（①②③）をvaulxのローカルDBに蓄積
   ↓
Agent（vaulx） → 即時送信試行（1tx = 1署名セット）
   │       └── fee payer = 当該広告主のデポジットプール
   ↓
...
広告主のbudgetからの支出：
  ├── CPM報酬
  │     ├── Screener: declared_share（max_screener_share以下）
  │     └── Curator:  残り（= 100% - declared_share - protocol_fee）
  ├── プロトコル手数料: 0.5-1%（Protocol Treasury）
  └── オンチェーン記録のガス代（fee payer）

参加者別コスト：
  広告主:   budget全額（報酬 + 手数料 + ガス代）
  Screener: ゼロ（SOL保有不要、ウォレットアドレスだけで参加）
  Curator:  ゼロ（同上）
  Agent:    ゼロ（同上、fee payerは広告主デポジット）
```
...
- **オンチェーン記録の主体：** 署名セット（①②③）を即時送信試行。失敗時はローカルDBに保存しリトライ
- Fee payerは広告主デポジットプールのため、Agent自身はガス代不要
...
- **Agent pubkeyの最低活動期間：** 生成直後のウォレットからの署名はクレーム対象外。ダミー量産の即時換金を構造的にブロック
...
vaulx（エージェントウォレットMCPサーバー）の送信設計。Phase 1では1tx = 1署名セット。バッチ処理は不要。

```
vaulx内部（Phase 1）:
  受信 → 署名③生成 → 即時送信試行（1tx = 1署名セット）
    成功 → 完了
    失敗 → ローカルDB（SQLite）に保存 → 次回リトライ
...
**Fee payer：** Solanaのプロトコルレベルでfee payerに対応。広告主のデポジットプールからガス代を支払い。Agent、Screener、CuratorはSOLを直接保有する必要がない。
...
- fee payer = 広告主デポジットプール
- 報酬分配ロジック（declared_shareに基づく自動分配）
- レートリミット（Curator単位 + 広告主budget単位のガス消費上限）+ 最低活動期間のハード制約
...
- Agentが1txでオンチェーン記録
- fee payerが広告主デポジットから支払われることを確認
- declared_shareに基づく報酬分配が正しく実行されることを確認

**Step 3：vaulx Solana対応 + 送信**
