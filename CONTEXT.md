...
広告主のdepositからの支出（per impression）:
  ...
  └── Submission Fee（CPM報酬とは完全に別枠）
        └── payer（tx提出者）に即時補填: submission_fee_lamports
        └── authority が ProtocolConfig で動的に調整可能
...
vaulx内部（Phase 1）:
  受信 → 署名③生成 → 送信先判定
    if (自分にSOLがある) → 直接 Solana RPC submit（fee payer = 自分）
    else → Screener relay or Protocol relay endpoint に fallback
...
- update_config instruction（authority がprotocol_fee_bps, treasury, submission_fee_lamportsを動的に変更可能）
